...
var openLightbox = function openLightbox(gallery, i, larges, descriptions) {
	...
	jQuery( '.lightbox-container img' ).trigger( "click" );

	var container_height = jQuery( '.lightbox-overlay .lightbox-container' ).height();
	var whole_body_height = jQuery( 'body' ).height();
	jQuery( '#lightbox' ).height(whole_body_height);
	jQuery( '.lightbox-overlay' ).height(container_height);
	
	jQuery("#gallery-container .col-md-6").each(function(){
		jQuery(this).find(".gallery-item .highlights_image").click(function(){
			var image_offset = jQuery(this).offset();
			jQuery( '.lightbox-overlay' ).css('top',image_offset.top);
		});
	});
...
var navigateLightbox = function navigateLightbox(lightboxElement, i, larges, descriptions) {
	...
	lightboxElement.addEventListener('click', function (e) {
		...
		jQuery('.lightbox-container .wrapper').remove();
		description.innerHTML = '<div class="wrapper"><div class="title">' + descriptions[i].split("~")[0] +'</div><div class="desc">'+ descriptions[i].split("~")[1] + '</div></div>';
		counter.textContent = '' + (i + 1) + ' of ' + larges.length + '';
	});
};
